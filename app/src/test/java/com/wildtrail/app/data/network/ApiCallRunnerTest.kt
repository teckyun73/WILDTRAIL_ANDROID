...
class ApiCallRunnerTest {
    @Test
    fun runWithFactory_retriesIOExceptionOnceWhenRetryTransientIsEnabled() = runTest {
        val api = FakeWildTrailApi("http://10.0.2.2:8000")
        var callCount = 0

        val result = ApiCallRunner.runWithFactory(
    ...
    @Test
    fun runWithFactory_doesNotRetryIOExceptionWhenRetryTransientIsDisabled() = runTest {
        var callCount = 0
    ...
    @Test
    fun runWithFactory_fallsBackFromEmulatorHostToLocalhostAfterPrimaryFailure() = runTest {
    ...
    @Test
    fun runWithFactory_doesNotFallbackWhenBaseUrlIsNotEmulatorHost() = runTest {
...
private class FakeWildTrailApi(val baseUrl: String) : WildTrailApi {
